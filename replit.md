...
## Recent Changes
- 2026-05-12: Reward picker added to create-request form: 5 preset chips ($3/$5/$10/$15/$20), defaults to $5; submit button shows selected amount; reward is now fully user-controlled (was hardcoded at $5)
- 2026-05-12: Removed fake random distance chip from request-detail (was showing random 2–7km value on every render)
- 2026-05-12: Fixed payment-setup infinite loop: after WebView card success, now calls GET /api/payments/payment-status before refreshProfile so DB is updated (hasPaymentMethod=true) before navigating back
- 2026-05-12: seeker@lokat.app seed now sets hasPaymentMethod=true so dev testing bypasses payment gate
- 2026-02-27: Bug fixes & pre-publish prep: removed `expo/fetch` import from store.ts (used native fetch for all calls); fixed confusing "check completed requests" message on request-detail (now says "scroll down to view it"); app scheme changed from "myapp" to "lokat"; bundle IDs updated to "com.lokat.app" (iOS) and "com.lokat.app" (Android)
- 2026-02-27: Forgot password implemented: `POST /api/auth/forgot-password` generates 6-digit OTP, stores with 15min expiry, sends via Resend email; `POST /api/auth/reset-password` validates OTP and updates password; `resetToken` + `resetTokenExpiry` columns added to users table; "Forgot password?" link on login screen opens modal with 2-step flow (email → code + new password → success)
- 2026-02-27: Push notifications implemented: `expo-notifications` installed; `expoPushToken` column added to users table; `server/pushNotifications.ts` helper sends real device pushes via Expo Push API; all 7 notification events wired (new request, accepted, photo submitted, payment received, photo delivered, chat message, completion); `POST /api/auth/push-token` endpoint registers device token; `lib/usePushNotifications.ts` hook requests permission, saves token, handles tap-to-navigate; integrated into AppProvider; `expo-notifications` plugin added to app.json
- 2026-02-27: Native in-app card entry: replaced Stripe Checkout browser redirect with a WebView modal loading a Stripe.js card form hosted at `/card-setup`; backend endpoint `POST /api/payments/create-setup-intent` creates SetupIntent + returns client secret; card form sends postMessage to React Native on success; no browser redirect needed
- 2026-02-27: Real Stripe charge on photo submission: `complete-submission` now retrieves seeker's saved payment method and creates + confirms PaymentIntent with `confirm: true, off_session: true`; returns 402 with clear error codes if card fails
- 2026-02-27: Ratings system fully removed: `ratings` table dropped from schema, storage methods/interface/routes removed, `averageRating`/`totalRatings` purged from seed + profile route
- 2026-02-27: TypeScript fixes: `request.notes` → `request.note` in camera screen; `scheduledDate?` added to PhotoRequest type; seed.ts cleaned of removed columns
